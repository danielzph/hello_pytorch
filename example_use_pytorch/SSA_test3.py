...
def SSA(pop,M,cc,dd,dim):
    ...
    fMax = np.max(fit[:,0])   # fMax表示全局最优适应值，生产者能量储备水平取决于对个人适应度值的评估
    # bestI = np.argmin(fit[:,0])
    bestI = np.argmax(fit[:, 0])
    bestX = X[bestI,:] # bestX表示fMax对应的全局最优位置的变量信息
    ...
    for t in range(M): # 迭代更新
        ...
        fmin = np.min(pFit[:, 0])  # 取出最小的适应度值
        # B = np.argmax(pFit[:,0])  # 取出最大的适应度值得下标
        B = np.argmin(pFit[:, 0])  # 取出最小的适应度值得下标
        worse = X[B,:]  # 最差适应度
        ...
        for j in range(len(b)):
            ...
            if pFit[sortIndex[0, b[j]], 0] > fMax:
                ...
                X[sortIndex[0, b[j]], :] = pX[sortIndex[0, b[j]], :] + (2 * np.random.rand(1) - 1) * np.abs(
                    pX[sortIndex[0, b[j]], :] - worse) / (pFit[sortIndex[0, b[j]]] - fmin + 10 ** (-50))
        ...
        Convergence_curve[0,t] = fMax
        # print(fMin)
        print(fMax)
        print(bestX)
    # return fMin, bestX, Convergence_curve
    return fMax,bestX,Convergence_curve
...
[fMax,bestX,SSA_curve]=SSA(pop,M,cc,dd,dim)
